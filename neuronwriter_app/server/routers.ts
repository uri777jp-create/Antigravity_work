...
## 執筆方針
1. **質問への回答**: 見出しが疑問形（「〜ですか？」「〜とは？」）の場合、その疑問に対する明確な答えを冒頭に記述してください。
2. **意図の汲み取り**: 見出しが抽象的な場合（例：「使い方は？」など）は、記事全体の文脈（今回は${input.keywords?.join(" ") || "このトピック"}に関連する内容）として具体的に解釈して補完してください。
3. **事実に基づく**: 検索結果に含まれない情報は書かず、情報不足時はその旨を簡単に触れてください。

## 執筆ルール
1. 読者に語りかけるような、丁寧かつ読みやすい文体（です・ます調）で書いてください。
2. 適切なHTMLタグ（<p>, <ul>, <li>, <strong>など）を使用して構造化してください（hタグは不要）。
3. 300文字〜600文字程度で簡潔にまとめてください。
4. 出典への言及（例：「出典1によると〜」）は文脈上自然であれば含めても良いですが、必須ではありません。
...
## 取材メモ（Web検索結果）
※記事の舞台設定は「${currentYear}年」です。
ただし、**文中で「${currentYear}年」という言葉を過剰に繰り返さないでください。**
読者にとって現在は${currentYear}年であることが前提であるため、わざわざ年号を強調する必要がない箇所では、「現在」「昨今」「最新の」といった自然な表現に言い換えてください。
${searchContext}
...
5. **年号の抑制**: 「${currentYear}年現在」といったフレーズは、記事全体で重要な節目となる1〜2箇所（導入部やまとめなど）に留め、各見出しや本文中で連呼しないよう注意してください。
